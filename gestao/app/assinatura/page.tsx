...
export default function SubscriptionPage(){
  ...
  const essentialCheckout=getCheckoutUrl("essential");
  const professionalCheckout=getCheckoutUrl("professional");
  ...
  const isTrial=workspace.business.status==="trial"&&!subscription;
  const essentialCurrent=!isTrial&&workspace.business.plan==="essential";
  const professionalCurrent=!isTrial&&workspace.business.plan==="professional";
  ...
  return <AppShell business={workspace.business} user={workspace.user} clientCount={clientCount} memberCount={memberCount}><main className="module-content">
    ...
    <div className="notice"><strong>Status atual:</strong> {subscription?`${subscription.status} via ${subscription.provider}`:isTrial?"período de avaliação":"sem assinatura registrada"}. O Nassus Gestão não armazena número de cartão ou credenciais bancárias.</div>
    {(!essentialCheckout||!professionalCheckout)?<div className="notice">Os checkouts da Cakto ainda não foram configurados neste ambiente. Os planos e o banco já estão prontos; basta inserir os links oficiais de checkout na configuração da hospedagem.</div>:null}
    <div className="plan-cards">
      <article className={`pricing-card ${essentialCurrent?"featured":""}`}><span className="eyebrow">ESSENCIAL</span>{essentialCurrent?<span className="recommended">PLANO ATUAL</span>:null}<h2>Para começar organizado</h2><div className="price">R$ 39,90 <small>/mês</small></div><ul><li>Até 2 usuários</li><li>Até 90 clientes</li><li>Agenda e serviços</li><li>Financeiro</li><li>Orçamentos</li><li>Dashboard e relatórios básicos</li><li>Uso no computador e celular</li></ul>{essentialCurrent?<button className="secondary" disabled>Seu plano atual</button>:workspace.business.plan==="professional"&&!isTrial?<button className="secondary" disabled>Downgrade pela central em breve</button>:essentialCheckout?<a className="secondary" href={essentialCheckout} target="_blank" rel="noreferrer">Assinar Essencial na Cakto</a>:<button className="secondary" disabled>Checkout ainda não configurado</button>}</article>
      <article className={`pricing-card ${professionalCurrent?"featured":""}`}><span className="eyebrow">PROFISSIONAL</span><span className="recommended">MAIS COMPLETO</span><h2>Para empresas em crescimento</h2><div className="price">R$ 139,90 <small>/mês</small></div><ul><li>Até 10 usuários</li><li>Clientes ilimitados</li><li>Todos os recursos do Essencial</li><li>Equipe e permissões ampliadas</li><li>Mais capacidade operacional</li><li>Base pronta para automações avançadas</li><li>Suporte prioritário</li></ul>{professionalCurrent?<button className="primary" disabled>Seu plano atual</button>:professionalCheckout?<a className="primary" href={professionalCheckout} target="_blank" rel="noreferrer">Assinar Profissional na Cakto</a>:<button className="primary" disabled>Checkout ainda não configurado</button>}</article>
    </div>
