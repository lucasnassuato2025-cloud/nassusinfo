...
type TeamMember = { member_id:string; user_id:string; name:string|null; email:string; role:string; active:boolean };
type Appointment = { id:string; client_id:string|null; service_id:string|null; professional_user_id:string|null; starts_at:string; ends_at:string|null; status:"scheduled"|"confirmed"|"completed"|"cancelled"|"no_show"; notes:string|null };
...
export default function AgendaPage(){
  ...
  async function load(){try{const current=await requireWorkspace();if(!current)return;const from=new Date(Date.now()-86400000).toISOString();const [cq,sq,aq,mq]=await Promise.all([
    ...
    neonClient.from("appointments").select("id,client_id,service_id,professional_user_id,starts_at,ends_at,status,notes").eq("business_id",current.business.id).gte("starts_at",from).order("starts_at",{ascending:true}).limit(200),
    (neonClient as any).rpc("list_business_members",{p_business_id:current.business.id}),
  ]);if(cq.error)throw cq.error;if(sq.error)throw sq.error;if(aq.error)throw aq.error;if(mq.error)throw mq.error;setWorkspace(current);setClients((Array.isArray(cq.data)?cq.data:[]) as Client[]);setServices((Array.isArray(sq.data)?sq.data:[]) as Service[]);setAppointments((Array.isArray(aq.data)?aq.data:[]) as Appointment[]);setMembers(((Array.isArray(mq.data)?mq.data:[]) as TeamMember[]).filter(member=>member.active));}catch(reason){setError(friendlyWorkspaceError(reason));}finally{setLoading(false);}}
  useEffect(()=>{void load();},[]);

  const clientMap=useMemo(()=>new Map(clients.map(c=>[c.id,c.name])),[clients]);
  const serviceMap=useMemo(()=>new Map(services.map(s=>[s.id,s])),[services]);
  const memberMap=useMemo(()=>new Map(members.map(member=>[member.user_id,member.name||member.email])),[members]);
  ...
  return <AppShell business={workspace.business} user={workspace.user} clientCount={clients.length} memberCount={members.length||1}><main className="module-content">
    <div className="module-head"><div><span className="eyebrow">OPERAÇÃO</span><h1>Agenda</h1><p>Agendamentos ligados a clientes, serviços e profissionais da equipe.</p></div></div>
    ...
    <div className="module-grid"><section className="panel"><div className="panel-head"><div><span className="eyebrow">PRÓXIMOS ATENDIMENTOS</span><h2>Agenda operacional</h2></div></div><div className="agenda-list">{appointments.map(a=>{const date=new Date(a.starts_at);const service=a.service_id?serviceMap.get(a.service_id):null;const professional=a.professional_user_id?memberMap.get(a.professional_user_id):null;return <article className="agenda-item" key={a.id}><div className="agenda-time"><strong>{date.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}</strong><small>{date.toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit"})}</small></div><div className="agenda-body"><strong>{a.client_id?clientMap.get(a.client_id)||"Cliente":"Cliente removido"}</strong><small>{service?.name||"Sem serviço definido"} • {formatDateTime(a.starts_at)}</small>{professional?<small>Profissional: {professional}</small>:null}{a.notes?<small>{a.notes}</small>:null}</div><div className="agenda-actions"><span className={`badge ${a.status==="completed"?"green":a.status==="confirmed"?"blue":a.status==="cancelled"?"red":"orange"}`}>{({scheduled:"Agendado",confirmed:"Confirmado",completed:"Concluído",cancelled:"Cancelado",no_show:"Faltou"} as Record<string,string>)[a.status]}</span>{a.status==="scheduled"?<button className="mini-button ok" onClick={()=>void setStatus(a,"confirmed")}>Confirmar</button>:null}{["scheduled","confirmed"].includes(a.status)?<button className="mini-button ok" onClick={()=>void setStatus(a,"completed")}>Concluir</button>:null}{["scheduled","confirmed"].includes(a.status)?<button className="mini-button cancel" onClick={()=>void setStatus(a,"cancelled")}>Cancelar</button>:null}<button className="danger" onClick={()=>void remove(a)}>Excluir</button></div></article>})}{!appointments.length?<div className="empty-state"><strong>Nenhum agendamento</strong><p>Crie o primeiro compromisso usando o formulário ao lado.</p></div>:null}</div></section>
      <aside className="form-panel"><span className="eyebrow">NOVO AGENDAMENTO</span><h2>Reservar horário</h2><p>Escolha cliente, serviço, profissional e data. A duração é calculada pelo serviço.</p><form onSubmit={createAppointment}><div className="form-grid"><div className="field full"><label>Cliente *</label><select required value={clientId} onChange={e=>setClientId(e.target.value)}><option value="">Selecione</option>{clients.map(client=><option key={client.id} value={client.id}>{client.name}</option>)}</select></div><div className="field full"><label>Serviço</label><select value={serviceId} onChange={e=>setServiceId(e.target.value)}><option value="">Sem serviço específico</option>{services.map(service=><option key={service.id} value={service.id}>{service.name}{service.duration_minutes?` • ${service.duration_minutes} min`:""}</option>)}</select></div><div className="field full"><label>Profissional</label><select value={professionalUserId} onChange={e=>setProfessionalUserId(e.target.value)}><option value="">Sem profissional definido</option>{members.map(member=><option key={member.user_id} value={member.user_id}>{member.name||member.email}</option>)}</select></div><div className="field full"><label>Data e hora *</label><input type="datetime-local" required value={startsAt} onChange={e=>setStartsAt(e.target.value)}/></div><div className="field full"><label>Observações</label><textarea value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Informações importantes para o atendimento"/></div></div><div className="form-actions"><button className="primary" disabled={saving||!clients.length}>{saving?"Salvando...":"Criar agendamento"}</button></div></form></aside>
    </div>
  </main></AppShell>;
}
