...
async function loadBusinessesWithRetry(attempts:number){
  let lastError:unknown=null;let lastData:WorkspaceBusiness[]=[];
  for(let index=0;index<attempts;index+=1){
    const result=await neonClient.from("businesses").select(BUSINESS_COLUMNS).order("name",{ascending:true});
    if(!result.error){lastData=(Array.isArray(result.data)?result.data:[]) as WorkspaceBusiness[];if(lastData.length||index===attempts-1)return lastData;}
    else lastError=result.error;
    if(index<attempts-1)await sleep(Math.min(760,160+index*110));
  }
  if(lastError)throw lastError;return lastData;
}

export async function loadWorkspace(options:WorkspaceOptions={}):Promise<Workspace|null>{
  const user=await waitForCurrentUser({attempts:options.sessionAttempts??5,delayMs:140});if(!user?.email)return null;
  ...
  const storedId=window.localStorage.getItem(ACTIVE_BUSINESS_KEY);const business=businesses.find(item=>item.id===storedId)||businesses[0];
  if(business.id!==storedId)window.localStorage.setItem(ACTIVE_BUSINESS_KEY,business.id);return{user,business,businesses};
}
export async function waitForWorkspaceReady():Promise<Workspace|null>{return loadWorkspace({sessionAttempts:10,dataAttempts:7});}
export async function requireWorkspace():Promise<Workspace|null>{const workspace=await loadWorkspace({sessionAttempts:6,dataAttempts:5});if(!workspace){window.location.replace("/sign-in");return null;}if(!workspace.business){window.location.replace("/onboarding");return null;}return workspace;}
...
export function friendlyWorkspaceError(reason:unknown):string{
  ...
  if(/PUBLIC_BOOKING_RATE_LIMIT/.test(message))return"A agenda recebeu muitas solicitações em pouco tempo. Aguarde alguns minutos e tente novamente.";
  if(/PUBLIC_BOOKING_CONTACT_LIMIT/.test(message))return"Esse contato já possui vários agendamentos futuros. Revise a agenda antes de criar outro.";
  ...
  if(/EMAIL_NOT_VERIFIED/.test(message))return"Esse usuário precisa confirmar o e-mail antes de entrar na equipe.";
  if(/USER_NOT_FOUND/.test(message))return"Esse e-mail ainda não possui uma conta no Nassus Gestão.";
  if(/OWNER_BUSINESS_ALREADY_EXISTS/.test(message))return"Esta conta já possui uma empresa própria. Para participar de outra empresa, o acesso deve ser adicionado pela equipe dela.";
  ...
  if(/WORKSPACE_SYNC_FAILED|SESSION_SYNC/i.test(message))return"Sua sessão está sendo sincronizada. Tente novamente em alguns segundos.";
